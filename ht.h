...
public:
    ...
    HASH_INDEX_T next()
    {
        ...
        HASH_INDEX_T offset = dhstep_;

        if( this->numProbes_ >= this->m_ ) {
            return this->npos;
        }
        HASH_INDEX_T loc = (this->start_ + this->numProbes_*offset) % this-> m_;
        this->numProbes_++;
        return loc;
...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    if((double)(this->size_+this->deleted) / (double) this->CAPACITIES[mIndex_] >= this->alpha){
        resize();
    }
    ...
    if(table_[idx] != nullptr){
        ...
        if(table_[idx]->deleted){
            ...
            this->size_++;
            this->deleted--;
            //std::cout << "Updated formerly deleted item" << std::endl;
        }
        return;
    }


    HashItem* added = new HashItem(p);
    table_[idx] = added;
    this->size_++;
    this->totalInserts++;
    //std::cout << "Inserted new item. Current loading factor = "<< (double)(this->size_+this->deleted) / (double) this->CAPACITIES[mIndex_] << std::endl;
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    HASH_INDEX_T old = CAPACITIES[mIndex_];
    this->mIndex_++;
    if (mIndex_ >= 28) {
        //this->mIndex_--;
        throw std::logic_error("Hash table capacity exceeded");
    }
